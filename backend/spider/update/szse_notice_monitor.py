...
class SZSENoticeMonitor:
    """深交所公告监控类"""

    def __init__(self):
        """初始化监控器"""
        self.fetcher = SZSENoticeFetcher()
    ...
    async def fetch_yesterday_notices(self) -> List[dict]:
        ...
        print(f"Fetching SZSE notices for {yesterday_str}...")
        ...
        try:
            ...
            for notice in notices:
                # 使用公告ID作为唯一标识
                notice_id = notice.get("ann_id", "")
                if notice_id and notice_id not in self.processed_ids:
                    notice['_id'] = str(notice_id)
    ...
    async def run(self):
        """运行监控循环"""
        print("=" * 60)
        print("Starting SZSE Notice Monitor...")
        print("=" * 60)
        print("Configuration:")
        print("  - Fetch: Previous day's all notices")
        print("  - Interval: 1 hour")
        print("  - Source: Shenzhen Stock Exchange")
        print("=" * 60)
        ...
        try:
            ...
            print("\n" + "=" * 60)
            print("Stopping SZSE Notice Monitor...")
            print("=" * 60)
        except Exception as e:
            print(f"\nMonitor crashed: {e}")
            import traceback
