...
export const getEvents = (params: EventsQuery): Promise<PaginatedResponse<Event>> => {
  return api.get<any, PaginatedResponse<Event>>('/events', { params })
}

export const getEventById = (id: string) => {
  return api.get<any, Event>(`/events/${id}`)
}

export const createEvent = (data: Partial<Event>) => {
  return api.post<any, Event>('/events', data)
}

export const updateEvent = (id: string, data: Partial<Event>) => {
  return api.put<any, Event>(`/events/${id}`, data)
}
...
export const analyzeEvent = (id: string) => {
  return api.post<any, Event>(`/events/${id}/analyze`)
}

export const getEventsBySector = (sectorCode: string, limit = 50) => {
  return api.get<any, Event[]>(`/events/sector/${sectorCode}`, { params: { limit } })
}

export const getEventsByStock = (stockCode: string, limit = 50) => {
  return api.get<any, Event[]>(`/events/stock/${stockCode}`, { params: { limit } })
}
